    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
        for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
            ...
            if (index >= led_min && index < led_max && index != NO_LED) {
                ...
                if (keycode > KC_TRNS) {
                  if(keycode == MT_CTL_ESC || keycode == MT_CTL_MIN || keycode == MT_ALT_DLR || keycode == MT_ALT_EXC || keycode == LT1_ENTER || keycode == LT1_DELETE keycode == LT2_SPACE || keycode == LT2_BSPC) {
